...
### M1.5 每日打卡与激励（v1.1 新增）

**F1-04 → 每日打卡奖励弹窗**
- **触发条件**: 每日首次登录（由后端 `POST /auth/me/checkin` 返回 `isFirstLoginToday: true` 触发）
- **功能**:
  - 显示当前连续打卡天数（Streak）。
  - 显示累计跟读句子总数。
  - 显示用户当前头像（Avatar）。
  - 点击“Start Today's Adventure!”进入今日课程。

**验收标准**:
- 弹窗 UI 采用明快色彩，包含烟花（Confetti）装饰。
- 支持手动关闭及二次触发（点击 Header 奖牌图标）。
